/*
 * $Id: evolve2Rad2ConPN.c,v 1.6 2009/03/07 23:44:00 pzimmerman Exp $
 *
 ...
 * "F = ma & (1CpN ='big-o'((v/c)^2)^1 & (2CpN = 'big-o'/
 *
 */
...
int main ( int argc, char *argv[] )
{
  ...
  double r_0pn, r_1pn, r_2pn;                		
	...
	gsl_odeiv_control* solver_control
		= gsl_odeiv_control_standard_new( 0.0, eps_ode, 1.0, 1.0 );
	...
	if ( argc != 8 )
	{
		...
		fprintf( stderr, "usage: %s M eta f_init e con_pn rad_pn ode_eps\n", argv[0] );
		return 1;
	} 
	...
	con_pn_order = atoi( argv[5] ); /* conservative pn order 								*/
	rad_pn_order = atoi( argv[6] ); /* conservative pn order 								*/
	eps_ode  = atof( argv[7] );
  ...
  input_params.PNorder = rad_pn_order;
	...
	snprintf( fname, fname_len * sizeof(char), 
			"n_model_%dCpN_%dRpN_%4.2f_%4.2f_%4.2f_%4.2f_%g.txt", 
			con_pn_order,rad_pn_order, M, f_init, eta, e, eps_ode );
	fp = fopen( fname, "w" );
  ...
  /* set termination condition: LSO = Schwarzschild iseo */
	...
	while (1)
	{
		...
		/* chi and xi simplify expressions */
		chi = ( 1. - y[1] * cos(u) );
		xi = M * y[0];  
    ...
    {
			l = y[0]*t + ( pow(xi, 4./3.) / (8.* sqrt(1. - y[1]*y[1]) * chi) ) 
