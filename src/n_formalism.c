/*
 * $Id: n_formalism.c,v 1.4 2009/03/07 23:44:00 pzimmerman Exp $ 
 *
 * Program numerically evolves a system of ODE's 
 * and generates a (2.0-conservative pN) + (2.0-radiation pN) 
 * eccentric binary waveform. 
 * Conservative pN corrections can be crudely pictured as  
 * "F = ma & (1CpN ='big-o'((v/c)^2)^1) & (2CpN = 'big-o'/((v/c)^2)^2) & ...
 *
 */
 ...
 * 		- The leading order RR ODEs were derived without the adiabatic approximation
 * 			 using Lagrange's method of variation of parameters using the quasi-keplerian 
 * 			 parameterization in the ADM Gauge. (PRD 70 064928) 
 * 		- The higher pN RR ODEs are Adiabatic Evolution Equations in the Harmonic Gauge.
 *	 	- Geometrized units (G = c = 1) such that total mass is the scale. 
 */
		...
		void* params)
{
	...
	dn_dt_0pn =  (96. + 292.* e*e + 37.* pow(e,4)) / 
		( 5.* pow(e_factor, 7./2.));

	dn_dt_1pn = ( pow(xi, 2./3.) / (280.* pow(e_factor,9./2.) )) * 
		(20368. - 14784.*eta + (219880. - 159600.*eta) *e*e 
		+ (197022. - 141708.*eta) * pow(e,4) + (11717. - 8288.*eta) * pow(e,6) );  

	dn_dt_2pn = ( pow(xi, 4./3.) / (30240. * pow(e_factor, 11./2.)) ) * 
		( 12592864. - 13677408.*eta + 1903104.*eta*eta
			+ (133049696. - 185538528.*eta + 61282032.*eta*eta) * e*e + 
			(284496744. - 411892776.*eta + 166506060.*eta*eta) * pow(e,4)
			+ (112598442. - 142089066.*eta + 64828848.*eta*eta) * pow(e,6) + 
			(3523113. - 3259980.*eta + 1964256.*eta*eta) * pow(e,8)
			+ 3024.* (96. + 4268.* e*e + 4386.* pow(e,4) + 
				175.*pow(e,6)) * (5. - 2.*eta) * sqrt(e_factor) );


	de_dt_0pn = (304. + 121.* e*e) / (15.* pow( e_factor, 5./2.));

  de_dt_1pn = ( pow(xi, 2./3.) / (2520.* pow(e_factor,7./2.)) ) * 
  	( 340968. - 228704.*eta + (880632. - 651252.*eta) *e*e 
			+ (125361. - 93184.*eta) * pow(e,4) );

	de_dt_2pn = ( pow(xi, 4./3.) / (30240.* pow(e_factor, 9./2.)) ) *
		(20815216. - 25375248.*eta + 4548096.*eta*eta 
			+ (87568332. - 128909916.*eta + 48711348.*eta*eta) *e*e +
			(69916862. - 93522570.*eta + 42810096.*eta*eta) * pow(e,4)
			+ (3786543. - 4344852.*eta + 2758560.*eta*eta) * pow(e,6) + 
			1008.* (2672. + 6963.*e*e + 565.*pow(e,4)) * (5. - 2.*eta) 
			* sqrt(e_factor) );
...
int main ( int argc, char *argv[] )
{
  ...
  double r_0pn, r_1pn, r_2pn;                		
	...
	double dphi_dt_0pn, dphi_dt_1pn, dphi_dt_2pn;
	double k, k_1pn, k_2pn;
	double lambda, lambda_1pn, lambda_2pn;
	double W, W_0pn, W_1pn, W_2pn;
	...
	if ( argc != 7 )
	{
		...
		fprintf( stderr, "usage: %s M eta f_init e con_pn rad_pn\n", argv[0] );
		return 1;
	} 
	...
	con_pn_order = atoi( argv[5] );
	rad_pn_order = atoi( argv[6] );

  input_params.M = M;
  input_params.eta = eta;
  input_params.PNorder = rad_pn_order;

	fprintf( stderr, "M 	   = %4.2f M_sun\n", M );
	fprintf( stderr, "eta    = %.6f\n", eta );
	fprintf( stderr, "f_init = %4.2f Hz \n", f_init );
	fprintf( stderr, "e      = %4.2f\n", e );
	fprintf( stderr, "R-pN_order = %2d\n", rad_pn_order );
	fprintf( stderr, "C-pN_order = %2d\n", con_pn_order );

	/* create the file name and file pointer */
	snprintf( fname, fname_len * sizeof(char), 
			"n_model_waveforms_%dCpN_%dRpN_%4.2f_%4.2f_%4.2f_%4.2f.txt", 
			con_pn_order,rad_pn_order, M, f_init, eta, e );
	fp = fopen( fname, "w" );

#if 0
	snprintf( fname, fname_len * sizeof(char), 
			"evolve_%2.1fpN_%4.2f_%4.2f_%4.2f_%4.2f.dat", 
			"n_model_%dCpN_%dRpN_%4.2f_%4.2f_%4.2f_%4.2f.dat", 
	...
	/* 
	 *	variables needed for the root solving mechanism
	 */
  ...
  gsl_function F;
	struct ic_params n_params = {f_init, M, eta, e, rad_pn_order};
  ...
  do {
	...
	/*
	 * n_init = root of Eqn (9) in G&T
	 * omega_GW_init - n * (1-k),
	 * where k is a pN function  
	 */

	y[0] = n * T_SUN;
	y[1] = e;
	
	/* set the initial eccentric anomaly to zero */
	u = 0.0;

  /* set termination condition: Schwarzschild LSO */
	...
	while ( 1 )
	{
		...
		if ( con_pn_order == 0 ) 
			l = y[0] * t;
    else if ( con_pn_order == 1 )
			l = y[0] * t;
    else if ( con_pn_order == 2 ) 
    {
			l = y[0] * t +
				( pow(xi, 4./3.) / (8.* sqrt(1. - y[1]*y[1]) * chi) ) 
				* ( (15. - eta) * eta * y[1] * sin(u) * sqrt(1. - y[1]*y[1])
			 	 	+ 12.* (5. - 2.*eta) * v_minus_u * chi ); 
		}
    else 
			fprintf(stderr, "Error in conservative pN order.\n");

		if ( isnan(l) || isinf(l) )
		{
			fprintf( stderr, "l is %f\n", l );
			exit (1);
		}
    ...
    dphi_dt_1pn = dphi_dt_0pn * ( pow(xi, 2./3.) / ( 32.* (1. - y[1]*y[1]) * chi) ) *
		...
		h_plus 	= - ( (eta * g_si * M) / (c_si*c_si*c_si*c_si * R )) * 
			( (1.+ cos(iota)*cos(iota)) *
...
#endif
		fprintf( fp, "%32.16e %32.16e %32.16e %36.16e %36.16e\n", T_SUN * t, y[0], y[1], h_plus, h_cross);
