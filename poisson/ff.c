...
#define NUMPOINTS (65536)   /* number of data points (wave)  */
const float PIE = 3.14159265358979;
const float f0 = 200.;        /* peak frequency */

const char filename[] = "ff.data";  /* data file */

const float mass_1 = 1.4;     /* first mass (solar mass)   */
const float mass_2 = 2.5;     /* second mass (solar mass)  */

const float e_init = 0.65;     /* initial eccentricity */
const float t_final = 7.08;  /* duration             */

const float left_braket = 1.05;
const float right_braket = 1.20;
...
float p_tab[NUMPOINTS+1],       /* semilatus (tabulated)               */
      ...
      fitting_factor,           /* value of fitting factor             */
      cm_param_max;             /* chirp mass parameter at maximum     */
...
int main()
{
  ...
  float calc_ambig(float cm_parameter);
  ...
  mnbrak(&ax,&bx,&cx,&fa,&fb,&fc,calc_ambig);
  fitting_factor = -golden(ax,bx,cx,calc_ambig,tol,&cm_param_max); 

  printf("#   Fitting factor          = %f\n", fitting_factor);
  printf("#   CM parameter at maximum = %f\n", cm_param_max);
...
void file_results()
{
  ...
  fprintf(output_file, "%.2f  &  %.2f  &  %.1f  &  %.2f  &  %.4f  &  %.2f  &  %.4f  &  %.4f  \\\\ \n", 
          e_init, p_tab[1], f_final, p_tab[NUMPOINTS], e_tab[NUMPOINTS], 
          t_final, cm_param_max, fitting_factor);
...
float calc_ambig(float cm_parameter)
{
  ...
  float noise,f,f_min,f_max,ambiguity;
