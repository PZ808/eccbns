 ...
 *  $Id: gsl_solver.c,v 1.21 2009/05/09 19:35:49 pzimmerman Exp $
 *											 	
 */
...
double e_init = -1;
double f_init = -1;	/* we set the initial freq. f_init to f_min/3 = 13.3333 */
double m1 = -1;		
double m2 = -1;
...
int main( int argc, char* argv[] )
{
	...
	snprintf( fname, fname_len * sizeof(char), "gsl_solver_%s_%s_%s_%.1e.txt", ode_system_name, of_name, solver_type, eps );
	free( of_name );
	fp_out = fopen( fname, "w" ); 
#if 0
	snprintf( fname, fname_len * sizeof(char), "gsl_solver_%s_%s_intermediate_steps_%.1e.txt", ode_system_name, solver_type, eps );
...
#if 0
			/* output to file the intermediate values */
		        fprintf( fp_int, "%.16f %32.16e %32.16e %32.16e \n", t * t_scale, y[0], y[1], y[2] );
#endif
