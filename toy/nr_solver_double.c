...
double dxsav;	/* smallest interval for which results are stored 		*/
double *xp;  	/* vector of intermediate independent variable values 		*/
double **yp;	/* matrix of intermediate values of the dependedent variables 	*/ 
int kmax;	/* maximum number of steps to be stored				*/
int kount;      /* actual number of steps required to achieve hdid = h		*/  
int nrhs;

void derivs( double nr_t, double nr_y[], double nr_dydt[] )
{
	...
	nr_dydt[1] = (double) dydt[0];
	nr_dydt[2] = (double) dydt[1];
	nr_dydt[3] = (double) dydt[2];
...
void init_vals( double ystart[], double ode_params[] )
{
...
int write_output_nr( FILE *fp, double t, double t_scale, double nr_y[] )
{
	double y[3];	
	double params[2];
...
void ode_termination( double nr_t, double nr_y[] )
{
	double t = (double) nr_t;
	double y[3];
...
int main( int argc, char* argv[] )
{
	...
	int i_max;
	int n;		
	double *ystart;		/* initial values */  
	double *y;		/* stores rows of yp[1][1...kmax], yp[2][1...kmax], yp[3][1...kmax] */ 	
	double t; 		/* time step */
	double t_next;		/* the next time step */ 
	double t_interval; 	/* the difference t_max - t_min */  
	double hmin;		/* min value of integration step */
	double h1;		/* guess for the initial integration step value */
 	double ode_params[4];
	...
	snprintf( fname, fname_len * sizeof(char), "nr_solver_double_%s_%s_%s_%.1e.txt", ode_system_name, of_name, solver_type, eps);
	free( of_name );
	fp_out = fopen( fname, "w" ); 

#if 0
	snprintf( fname, fname_len * sizeof(char), "nr_solver_double_%s_%s_intermediate_steps_%.1e.txt", ode_system_name, solver_type, eps);
	...
	{	
		...
		dxsav    = (double) ( t_interval / kmax );  
		
		/* call the solver to get the next time step */
		
		if ( ! strcmp( "rkqs", solver_type ) )
			odeint ( ystart, N, (double)t, (double)t_next, (double)eps, 
