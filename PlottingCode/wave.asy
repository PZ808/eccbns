...
//draw(pic1, eccWave1, black);
draw(pic2, eccWave2, black);
...
real scale=(max0.x-min0.x)/(max2.x-min2.x); 
real shift=min0.x/scale-min2.x; 
 
transform t0 = pic0.calculateTransform(); 
transform t2 = pic2.calculateTransform(); 
transform T=xscale(scale*t0.xx)*yscale(t2.yy); 
