/* $Id: x_drive.c,v 1.15 2009/06/08 18:07:10 pzimmerman Exp $ */

/* Driver code to Generate the gravitational   *
 * wave signal from eccentric inspirals using  *
 * the x-based formalism of Hinder et al       *
 * (http://arxiv.org/abs/0806.1037). The       *
...
const double euler_phi   = 0.0;      /* detector's azimuthal angle       */
const double euler_psi   = 0.0;      /* polarization plane's polar angle */

const double c_si_pow_4 = C_SI*C_SI*C_SI*C_SI;  /* C_SI is speed of light */
...
int main( int argc, char *argv[] )
{
  ...
  double y[4];       /* array containing x, e, l, phi                  */
  double y_dot[4];   /* array containing x_dot, e_dot, l_dot, phi_dot  */

  /* vectors to store the data */

  double *t_vec;            /* time                  */
  double *x_vec;            /* PN expansion variable */
  double *e_vec;            /* eccentricity          */
  double *l_vec;            /* mean anomaly          */
  double *u_vec;            /* eccentric anomaly     */
  double *phi_vec;          /* orbital phase         */
  double *phi_dot_vec;      /* orbital frequency     */
  double *r_vec;            /* orbital separation    */
  double *r_dot_vec;        /* orbital velocity      */
	double *p_vec;            /* semi-latus rectum     */
  double *h_plus_vec;       /* GW plus polarization  */
  double *h_cross_vec;      /* GW cross polarization */
  double *h_vec;            /* GW signal             */ 
  double *x_coordinate_vec; /* x w.r.t. the C.O.M    */
  double *y_coordinate_vec; /* y w.r.t. the C.O.M    */

  /* parameters specified at command line */

  int con_pn_order;       /* conservative PN order */
  int rad_pn_order;       /* radiation PN order    */
  double mass1, mass2;    /* masses                */
  double e_init;          /* initial eccentricity  */
  double f_gw_init;       /* initial GW frequency  */
  double mean_anom_init;  /* initial mean-anomaly  */
  double ode_eps;         /* tolerance (relative)  */ 
  ...
  double x_final;
  double x_final_2pn;
  double p_final;     /* final semi-latus rectum   */
  double f_gw_isco;   /* GW frequency and isco     */
  double num_cycles;  /* total number of GW cycles */
  double total_mass;
  ...
  {                  
    ...
    /* print correct usage for command line arguments */
   ...
   *  References: K.S. Thorne "300 Years"           *
  ...
  /* Hardwire filenames for python slicing of *
   * massess and eccentricity.                */ 

  if ( mass1 <= 10.0 && mass2 <= 10.0 )
    snprintf( fname_base, fname_length * sizeof(char),
        "_%d_%d_%3.2f_%3.2f_%.2f_%2.1e.txt",
        con_pn_order, rad_pn_order, mass1, mass2, e_init, ode_eps );
  if ( mass1 <= 10.0 && mass2 >= 10.0 )
    snprintf( fname_base, fname_length * sizeof(char),
        "_%d_%d_%3.2f_%2.1f_%.2f_%2.1e.txt",
        con_pn_order, rad_pn_order, mass1, mass2, e_init, ode_eps );
  if ( mass1 >= 10.0 && mass2 <= 10.0 )
    snprintf( fname_base, fname_length * sizeof(char),
        "_%d_%d_%2.1f_%3.2f_%.2f_%2.1e.txt",
        con_pn_order, rad_pn_order, mass1, mass2, e_init, ode_eps );
#if 0
  ...
  gsl_odeiv_system solver_system = { eccentric_x_model_odes, NULL, 4, &ecc_params };
  ...
  for ( i = 2; i < max_i_reached-2; ++i )
  {
    ...
    fprintf( fp[0], "%d %24.16e %24.16e %24.16e %24.16e\n",
