...
int main( int argc, char *argv[] )
{
  ...
  u_vec[0] = pn_kepler_equation( con_pn_order, sym_mass_ratio,
      x_vec[0], e_vec[0], l_vec[0] );
  ...
  for ( i = 1; i < i_max; ++i )
  {
    ...
    if ( (rad_pn_order <= 2) && ((con_pn_order < 1) ? (p_vec[i] <= 6.0) : (x_vec[i] >= x_final)) )
    {
      ...
      fprintf( stderr, "-----------------------------------\n");
      fprintf( stderr, "Terminating at Schwarzschild-isco\n");
      fprintf( stderr, "-----------------------------------\n");

      fprintf( stderr, "m1 = %.3f\t m2 = %.3f\t fLow = %.3f\n" 
                       "eta = %.2f\t M = %.2f\t Mchirp = %.2f\n"
                       "isco freq = %.6e Hz\n"
                       "length = %.6e seconds\n"
                       "GW cycles = %.6f\n", 
                       mass1, mass2, f_gw_init, 
                       sym_mass_ratio, total_mass, 
                       pow(reduced_mass, 3./5.)*pow(total_mass, 2./5.), 
                       f_gw_isco/M_SUN_S, t * (total_mass*M_SUN_S), num_cycles );
