/* $Id: x_formalism_new.c,v 1.2 2009/03/28 20:06:32 pzimmerman Exp $ */
...
int main( int argc, char *argv[] )
{
   ...
   * solves for the initial conditions 
   * on the mean motion 
   */
  ...
  do {
	...
	/* initial orbital period in seconds from n_init */
	period = 2.0*M_PI / n;
	/* initial omega in M-sun from period */
  ...
  /* create the the file name and file pointer */
	snprintf(fname, fname_length * sizeof(char),
			"x_model_waveforms_%d_%d_%.1f_%.1f_%.1f_%.2f_%2.1e_%.1f_%g.txt", 
   ...
   */
   ...
   n_final = 1.0 / ( 6.0 * sqrt(6.0) * total_mass );
   /* terminate when  M * omega_gw = 0.1 */
   x_final = 0.341995;
  ...
  i_max = (unsigned long int) ceil( t_interval / dt );
    ...
    {
    	t *= T_SUN; 
			fprintf( fp, "%24.16e %24.16e %24.16e %24.16e %24.16e %24.16e %24.16e %24.16e\n", 
